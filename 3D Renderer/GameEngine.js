...
const ScreenWidth = () => { return canvas.width; }
/**Returns Canvas Height */
const ScreenHeight = () => { return canvas.height; }
...
const GetMouseX = () => { return mousePos.x; }
/**Returns the Y position of the mouse on the canvas. */
const GetMouseY = () => { return mousePos.y; }
...
var Game = {
    ...
    ConstructCanvas: function (w, h, pixelS = 1) {
        ...
        this.tabTitle = document.getElementsByTagName("title")[0];
        this.title = this.tabTitle.innerText;
    ...
    Start: function () {
        ...
        document.addEventListener("keydown", function (e) {
            ...
            if (e.key === 'ArrowUp' || e.key === 'ArrowDown')
                e.view.event.preventDefault();  //Prevent Up/Down Keys from scolling page
        });
    ...
    run: function (fTimeStamp = 0) {
        ...
        if (this.update(this.fElapsedTime)) {
            //update title
            this.tabTitle.innerText = this.title + " - FPS: " + (1 / this.fElapsedTime).toFixed(1);
            //Reset Key States
            for (v of changedKeys) {
                v.bReleased = false;
                v.bPressed = false;
            }
            changedKeys = [];
...
function FillTriangle(x1, y1, x2, y2, x3, y3, color = "#000") {
    c.fillStyle = color;
    c.beginPath();
    c.moveTo(x1, y1);
    c.lineTo(x2, y2);
...
function DrawTriangle(x1, y1, x2, y2, x3, y3, color = "#000") {
    c.strokeStyle = color;
    c.beginPath();
    c.moveTo(x1, y1);
    c.lineTo(x2, y2);
